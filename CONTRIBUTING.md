...
`skills/` and `.claude-plugin/` are installed from GitHub (Claude Code plugin marketplace / `npx skills add`), not from the npm tarball. The SKILL.md is a public contract: if you change MCP tool names, CLI flags, or the CLI JSON shape, update `skills/video/SKILL.md`, `README.md`, and the "Using this project as a tool" section of `AGENTS.md` in the same PR.
